    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $row) {
            ...
            try {
                ...
                if ($class_numeral >= 9) {
                    // For classes 9 and above: General + specific academic_group (compulsory only)
                    ...
                    // For classes below 9: only matching academic_group (compulsory only)
                    $subjects = Subject::where('class_id', $row['class_id'])->where('subject_type', 'compulsory')->where('academic_group', $row['academic_group'])->pluck('id');
                }
                ...
                // Handle main_optional_subject (is_4th_subject = 0)
                $mainOptionalSubject = $row['main_optional_subject'] ?? null;
                $this->enrollOptionalSubject($student->id, $mainOptionalSubject, false, $enrolledSubjectIds, $rowNumber);

                // Handle 4th_subject (is_4th_subject = 1)
                $fourthSubject = $row['4th_subject'] ?? null;
                $this->enrollOptionalSubject($student->id, $fourthSubject, true, $enrolledSubjectIds, $rowNumber);
