...
var KTStudentAttendance = function () {
                  ...
                  .then(function (response) {
                        ...
                        if (response.batches && response.batches.length > 0) {
                              response.batches.forEach(function (batch) {
                                    ...
                                    // option.textContent = batch.name + ' (' + batch.branch_name + ')';
      ...
      var renderOffDayWarning = function (dayName) {
            offDayWarning.innerHTML =
                  ...
                  '<span class="fs-7">' +
                  'Today (<strong>' + dayName + '</strong>) is the official off-day for this batch. ' +
      ...
      var renderStudentTable = function (students) {
            var html =
                  ...
                  '<th class="text-center min-w-350px">Attendance Status</th>' +
                  '<th class="min-w-200px pe-4 rounded-end">Remarks</th>' +
                  '</tr>' +
                  '</thead>' +
                  '<tbody>';
            ...
            students.forEach(function (student, index) {
                  ...
                  html +=
                        '<tr data-student-id="' + student.id + '" class="attendance-row">' +
                        ...
                        '<span class="text-muted fw-semibold fs-7">ID: ' + student.student_unique_id + '</span>' +
                        '</div>' +
                        '</div>' +
                        '</td>' +
                        '<td>' +
                        ...
                        'placeholder="Add remarks (optional)" value="' + remarks + '">' +
                        '</td>' +
                        '</tr>';
            });
      ...
      var saveAttendance = function () {
            ...
            if (hasValidationError) {
                  showAlert('error', 'Incomplete Attendance',
                        'Please select a status (Present, Late, or Absent) for all students highlighted in red.');
                  return;
            }
