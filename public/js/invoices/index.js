...
// Class definition - Edit Invoice Modal (Fixed)
var KTEditInvoiceModal = function () {
    ...
    var handleEditClick = function () {
        document.addEventListener('click', function (e) {
            ...
            // Show loading state in display fields
            ...
            fetch(`/invoices/${invoiceId}/view-ajax`, {
                ...
                .then(data => {
                    ...
                    // Set student display with name and ID from API response
                    const studentDisplay = document.getElementById('edit_student_display');
                    if (studentDisplay) {
                        const studentName = invoice.student_name || 'Unknown';
                        const studentUniqueId = invoice.student_unique_id || '';
                        studentDisplay.innerHTML = `<span class="fw-semibold">${studentName}${studentUniqueId ? ' (' + studentUniqueId + ')' : ''}</span>`;
                    }

                    // Set invoice type display
                    const typeDisplay = document.getElementById('edit_invoice_type_display');
                    if (typeDisplay) {
                    ...
                    // Set month_year display
                    const monthYearDisplay = document.getElementById('edit_month_year_display');
                    if (monthYearDisplay) {
...
var KTAddCommentModal = function () {
    ...
    // Load comments for the invoice
    var loadComments = function (invoiceId) {
        const commentsLoading = document.getElementById('comments_loading');
        ...
        const url = routeGetComments.replace(':id', invoiceId);
            ...
            .then(data => {
                ...
                if (data.success && data.comments && data.comments.length > 0) {
                    ...
                    data.comments.forEach(comment => {
                        commentsHtml += `
                            <div class="d-flex flex-column mb-4 pb-3 border-bottom border-gray-200">
                                <div class="d-flex align-items-center mb-1">
                                    <span class="fw-bold text-gray-800 fs-6 me-2">${comment.commented_by}</span>
    ...
    var handleAddCommentClick = function () {
        document.addEventListener('click', function (e) {
            ...
            // Update modal title
            const titleEl = document.getElementById('kt_modal_add_comment_title');
            if (titleEl) {
                titleEl.textContent = `Comments - Invoice ${invoiceNumber || invoiceId}`;
            }
    ...
    // Handle delete comment - Disabled
    ...
    // Handle modal close/cancel
    var handleModalClose = function () {
        const cancelButton = element.querySelector('[data-kt-add-comment-modal-action="cancel"]');
        const closeButton = element.querySelector('[data-kt-add-comment-modal-action="close"]');
