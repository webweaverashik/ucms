...
var KTCreateStudent = (function () {
    ...
    var secondaryClassSelect;
    var secondaryClassesList = []; // Store fetched secondary classes data
    ...
    var initStepper = function () {
        ...
        stepperObj.on("kt.stepper.next", function (stepper) {
            ...
            if (currentStep === 3) {
                ...
                // Generate dynamic fee inputs for Step 4
                generateSecondaryClassFeeInputs();
            }
    ...
    var loadReferredByData = function (type) {
        ...
        $.ajax({
            ...
            success: function (data) {
                referredBySelect.empty().append('<option value="" disabled selected>Select the person</option>');
                $.each(data, function (index, person) {
                    var displayText = person.name;
                    if (person.student_unique_id) {
                        displayText += ' (' + person.student_unique_id + ')';
                    }
                    referredBySelect.append('<option value="' + person.id + '">' + displayText + '</option>');
                });
    ...
    var initSubjects = function () {
        ...
        secondaryClassSelect = $('select[name="secondary_classes[]"]');
        ...
        // Init Secondary Class Select2
        if (secondaryClassSelect.length > 0) {
            secondaryClassSelect.select2({
                placeholder: "Select secondary classes",
                allowClear: true,
                closeOnSelect: false
            });
        }
    ...
    var renderInstitutions = function (institutions) {
        ...
        institutions.forEach(function (institution) {
            institutionSelect.append($('<option></option>').val(institution.id).text(institution.name + ' (EIIN: ' + (institution.eiin_number || 'N/A') + ')'));
        });

        institutionSelect.prop('disabled', false).trigger('change');
    };

    var loadSecondaryClasses = function () {
        var classId = classSelect.val();
        secondaryClassSelect.empty().trigger("change");

        if (!classId) return;

        // Note: Route must be defined in web.php as: Route::get('students/class/{id}/secondary-classes', ...)
        $.ajax({
            url: '/students/class/' + classId + '/secondary-classes',
            type: 'GET',
            dataType: 'json',
            success: function (response) {
                secondaryClassesList = response.data || []; // Store data for later use
                if (response.success && response.data.length > 0) {
                    response.data.forEach(function (item) {
                        var text = item.name + ' - ' + item.fee_amount + ' Taka (' + (item.payment_type || '') + ')';
                        var newOption = new Option(text, item.id, false, false);
                        secondaryClassSelect.append(newOption);
                    });
                }
                secondaryClassSelect.trigger('change');
            },
            error: function (e) {
                console.error("Error loading secondary classes", e);
            }
        });
    };
    ...
    var renderSubjects = function (subjects, currentGroup, hasOptional, selectionMode) {
        ...
        var hasCompulsory = (subjects.general_compulsory && subjects.general_compulsory.length) || (subjects.group_compulsory && subjects.group_compulsory.length);
    ...
    var validateSubjectSelection = function () {
        // Bypass if only_secondary_class is checked
        if ($('#only_secondary_class').is(':checked')) {
            return { valid: true };
        }
    ...
    var validateOptionalSubjects = function () {
        ...
        var hasOptionalSection = $('.optional-4th-radio').length > 0 || $('.optional-main-radio').length > 0;
            ...
            validator.validate().then(function (status) {
                if (status === "Valid") {
                    ...
                    for (var pair of formData.entries()) {
                        if (pair[0].startsWith('subjects[') || pair[0] === 'optional_main_subject' || pair[0] === 'optional_4th_subject') {
    ...
    var showErrors = function (errors) {
        ...
        errors.forEach(function (error) {
            ...
            errorElement.classList.add("alert", "alert-dismissible", "bg-light-danger", "border", "border-danger", "border-dashed", "d-flex", "flex-column", "flex-sm-row", "w-100", "p-5", "mb-10");
    ...
    // NEW: Handle "Only Secondary Class" Checkbox
    var initOnlySecondaryClassHandler = function() {
        var checkbox = $('#only_secondary_class');
        var admissionFeeInput = $('input[name="student_admission_fee"]');
        var tuitionFeeInput = $('input[name="student_tuition_fee"]');
        var subjectList = $('#subject_list');
        var subjectNotice = subjectList.prev('.notice');

        if(checkbox.length > 0) {
            checkbox.on('change', function() {
                if($(this).is(':checked')) {
                    // Set values to 0 and make readonly
                    admissionFeeInput.val(0).prop('readonly', true).addClass('bg-secondary');
                    tuitionFeeInput.val(0).prop('readonly', true).addClass('bg-secondary');

                    // Disable subject selection
                    subjectList.css({'opacity': '0.5', 'pointer-events': 'none'});
                    if(subjectNotice.length) subjectNotice.css({'opacity': '0.5'});
                } else {
                    // Make editable again
                    admissionFeeInput.prop('readonly', false).removeClass('bg-secondary');
                    tuitionFeeInput.prop('readonly', false).removeClass('bg-secondary');
                    
                    // Enable subject selection
                    subjectList.css({'opacity': '1', 'pointer-events': 'auto'});
                    if(subjectNotice.length) subjectNotice.css({'opacity': '1'});
                }
            });
        }
    };

    var generateSecondaryClassFeeInputs = function() {
        var container = $('#secondary-class-fees-container');
        container.empty();
        
        if (!secondaryClassSelect) return;
        
        var selectedIds = secondaryClassSelect.val(); // array of strings
        if (!selectedIds || selectedIds.length === 0) return;
        
        var html = '';
        selectedIds.forEach(function(id) {
            // Find class data from the stored list
            var cls = secondaryClassesList.find(function(c) { return c.id == id; });
            if (cls) {
                html += '<div class="col-md-3 mb-7">' +
                        '<label class="form-label required">' + cls.name + ' Fee</label>' +
                        '<input type="number" name="secondary_class_fees[' + id + ']" class="form-control form-control-solid" value="' + cls.fee_amount + '" min="0" placeholder="Enter fee amount" required />' +
                        '</div>';
            }
        });
